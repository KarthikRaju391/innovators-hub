...
function Products({ orders, initialCursor }) {
	...
	const loadMoreOrders = async () => {
		...
		const res = await fetch(
			`/api/users/${session?.user.id}/orders?delivered=true&cursor=${cursor}`,
			{
				method: "GET",
				headers: {
	...
	var tblContent =
		...
		loadedOrders.map((order) => (
			<tr key={order.id} className="row animate__animated animate__fadeInUp">
				...
				<td className="col">
					<Button
						onClick={() => router.push(`/user/purchase/ordershistory/${order.id}`)}
						size={SIZE.mini}
						shape={SHAPE.pill}
					>
                        View Order 
...
export async function getServerSideProps(context) {
	...
	const res = await fetch(
		`${process.env.NEXT_APP_URL}/api/users/${session.user.id}/orders?delivered=true`,
		{
			method: "GET",
			headers: {
				"Content-Type": "application/json",
