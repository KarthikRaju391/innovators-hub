...
function SideNavPostalAdmin({open, handleOpen}) {
    ...
    return (
        ...
        >
            ...
            { session.data ? <button onClick={()=>signOut({callbackUrl: '/'})} className="mt-3 pt-2 ml-6 pl-7 text-base">Sign Out</button> : <button onClick={()=>signIn()} className="mt-3 pt-2 ml-6 pl-7 text-base">Sign In</button> }
        </Drawer>
    );
}
